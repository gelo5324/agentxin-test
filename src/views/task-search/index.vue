<template>
    <div class="task-search-view">
        <div class="task-search-view-header">
            <van-dropdown-menu>
                <van-dropdown-item v-model="tabActive" :options="tabsData" @open="dropOpenHandler" @close="dropCloseHandler" @change="dropChangeHandler" />
            </van-dropdown-menu>
            <div class="header-search-view" v-show="!dropVisible">
                <van-search v-model="filterParams.name" placeholder="请输入" @search="onSearchHandle" />
            </div>
            <div class="header-cancel-text" @click="cancelHandler" v-show="!dropVisible">取消</div>
        </div>
...
const tabActive = ref(localStorage.getItem('taskTabActive')||'a');
const apiEvents = {
    a: GetDoneData,
    b: GetUnreadData,
    c: GetFinishData,
