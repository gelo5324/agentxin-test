  ...
  <el-form
    ...
    <el-scrollbar v-if="formItems.length>0">
      <template v-for='(x, i) in formItems'>
        <el-form-item v-if="x.hiddenHandler ? x.hiddenHandler(x,form) : !x.isHidden" :class="x.class || ''" :key='x.id || i' :label='x.label' :label-width="x.labelWidth" :prop="x.prop" :style="x.style || {}">
